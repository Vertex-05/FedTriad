...
class ExogenousDetectionClientValidation:
    ...
    def __distance_global_model_final_metric(distance_type: str, prediction_matrix,
                                             ...
                                             own_index):
        """
        Calculates the distance matrix containing the metric for ExogenousDetection
        ...
        for s_i, s in enumerate(prediction_matrix):
            g = prediction_global_model[s_i] #是一个列表，存储了全局模型对所有样本的预测结果
            for m_i, m in enumerate(s):
                for l_i, l in enumerate(m):
                    distance = DistanceHandler.get_distance(distance_type, l, g[l_i])  
        ...
        for label, sample_list in sample_indices_by_label.items():
            ...
            global_distance_matrix_for_label_helper = [
                ...
                range(model_count)]#是一个三维嵌套列表，用于存储当前标签下的样本距离
                                #维度为 model x layer x sample
            s_i_new = 0
    ...
    def __predict_for_single_model(model, local_data, device):
        ...
        - The labels for all samples in the client's training dataset(as a list)
            ...
            for layer_output_index, layer_output_values in enumerate(output):
                for idx in range(target.shape[0]):
                    ...
                    if layer_output_index == 0:
                        ...
                        #expected_predictions 是当前循环内部的临时变量
                        #number_of_previous_samples 在外层 for 循环中是累加更新的，所以可以用来判断我们总共处理了多少样本
                    assert_msg = f'{len(predictions)} vs. {sample_idx} ({idx} {batch_id} '
                    assert_msg += f'{layer_output_index} {number_of_previous_samples})'
                    assert len(predictions) == expected_predictions, assert_msg #检查 predictions 列表的长度是否等于预期的样本数量。
                    ...
                    assert len(predictions[sample_idx]) == layer_output_index, assert_msg #检查当前样本的层数是否等于当前层的索引
                    #通过断言（assert）检查 predictions 列表的结构是否符合预期，并在断言失败时提供详细的错误信息（assert_msg），便于调试
    ...
    def __do_predictions(models, global_model, local_data, device):
        ...
        for model_index, model in enumerate(models):
            predictions, _, _ = ExogenousDetectionClientValidation.__predict_for_single_model(model,
                                                                                      local_data,
                                                                                      device)
            for sample_index, layer_predictions_for_sample in enumerate(predictions):
                ...
                all_models_predictions[sample_index].append(layer_predictions_for_sample) #将当前模型对该样本的预测结果追加到对应样本的存储位置中
        tmp = ExogenousDetectionClientValidation.__predict_for_single_model(global_model, local_data,
                                                                    device)
    ...
    def __prune_poisoned_models(num_layers, total_number_of_clients, own_client_index,
        ...
        for distance_type in distances_by_metric.keys():
            ...
            #初始化一个二维矩阵，维度为 total_number_of_clients x layer_length
            label_count = 0
            ...
            client_indices = [x for x, value in enumerate(dist_matrix_m_l) if
                              x != own_client_index]
            #生成一个列表，包含所有客户端的索引，但排除了当前客户端的索引
            #过滤掉当前客户端的索引 own_client_index，因为当前客户端不需要对自己的模型进行检测
            pruned_indices = []
            ...
            while has_malicious_model and new_round_needed:
                ...
                for value in cluster_input_plain:
                    # Split the samples into two groups分成两组并且计算绝对值
    ...
    def validate_models(global_model, models, own_client_index, local_data, device):
        tmp = ExogenousDetectionClientValidation.__do_predictions(models, global_model, local_data, device)
        ...
        for dist_type in [DistanceMetric.COSINE, DistanceMetric.EUCLIDEAN]:
            calculated_distances = ExogenousDetectionClientValidation.__distance_global_model_final_metric(
                dist_type,
                prediction_matrix,
        ...
        result = ExogenousDetectionClientValidation.__prune_poisoned_models(num_layers, len(models),
                                                                    own_client_index,
                                                                    distances_by_metric)
        return result
