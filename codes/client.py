...
# ---- 1. Trigger injection function (keep ExogenousDetection official 6×6 setting) ----
def trigger_single_image(image):
    ...
    """
    # Ensure MEAN and STD are defined at the top of client.py, or define them locally here
    MEAN = torch.tensor([0.4914, 0.4822, 0.4465])
    STD  = torch.tensor([0.2023, 0.1994, 0.2010])
    
    color = (torch.tensor((1.0, 0.0, 0.0)) - MEAN) / STD
    color = color.to(image.device).type(image.dtype)
    # Overwrite top-left 6×6 region
    image[:, 0:6, 0:6] = color.repeat((6, 6, 1)).permute(2, 1, 0)
    return image

# ---- 2. Data poisoning function ----
...
# ---- 3. Helper function: convert DataLoader to TensorDataset ----
...
# ---- 4. Revised Client_Backdoor class ----
class Client_Backdoor(Device):
    def __init__(self, model_name, optimizer_fn, loader, idnum=0, num_classes=10, dataset='cifar10',
        ...
        # Immediately generate poisoned loader at initialization
        if prepare_poisoned:
            try:
    ...
    def prepare_poisoned_loader(self, max_samples=None):
        # 1. Convert existing loader to TensorDataset
        ...
        # 2. [Key fix] Smart data extraction (compatible with standard PyTorch and custom utils.TensorDataset)
        imgs, labs = None, None
