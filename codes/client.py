...
def poison_data(samples_to_poison, labels_to_poison, pdr=0.5, target_label=2):
    """
    Poison a batch of samples (NxCxHxW).
    """
    if pdr == 0:
        return samples_to_poison, labels_to_poison
    ...
    if num_to_poison == 0 and dataset_size > 0:
        num_to_poison = 1 # 至少投毒一个
...
# ---- 3. DataLoader 转 TensorDataset 辅助函数 ----
def dataloader_to_tensor_dataset(loader, max_samples=None, device='cpu'):
    imgs = []
    labs = []
    cnt = 0
    ...
    def __init__(self, model_name, optimizer_fn, loader, idnum=0, num_classes=10, dataset='cifar10',
        ...
        if prepare_poisoned:
            try:
                ok = self.prepare_poisoned_loader(max_samples=max_poison_samples)
                if not ok:
                    print(f"[Client_Backdoor] client {self.id}: prepare_poisoned_loader failed.")
            except Exception as e:
                print(f"[Client_Backdoor] Error in prepare_poisoned_loader: {e}")
    
    def synchronize_with_server(self, server):
        self.server_state = server.model_dict[self.model_name].state_dict()
        self.model.load_state_dict(self.server_state, strict=False)

    def prepare_poisoned_loader(self, max_samples=None):
        """
        [修正版] 兼容 utils.py 中自定义 TensorDataset 的投毒数据准备函数
        """
        # 1. 将现有 loader 转换为 TensorDataset
        tensor_ds = dataloader_to_tensor_dataset(self.loader, max_samples=max_samples, device='cpu')
        if tensor_ds is None:
            print(f"[Client_Backdoor {self.id}] Failed to convert loader to TensorDataset.")
            return False

        # 2. [关键修改] 智能获取数据 (兼容标准 PyTorch 和自定义 utils.TensorDataset)
        imgs, labs = None, None
        
        # 尝试 A: 标准 PyTorch TensorDataset (.tensors)
        if hasattr(tensor_ds, 'tensors'):
            imgs, labs = tensor_ds.tensors
        
        # 尝试 B: 自定义 utils.TensorDataset (.images, .labels)
        elif hasattr(tensor_ds, 'images') and hasattr(tensor_ds, 'labels'):
            imgs = tensor_ds.images
            labs = tensor_ds.labels
        
        # 尝试 C: 都没有？打印错误
        if imgs is None or labs is None:
            print(f"[Client_Backdoor {self.id}] Error: Unknown TensorDataset format. Attributes: {dir(tensor_ds)}")
            return False

        # 3. 执行投毒 (poison_data 返回的是 Tensor)
        # 注意：ensure data is on CPU for manipulation
        imgs_cpu = imgs.detach().cpu()
        labs_cpu = labs.detach().cpu().long()
        
        poisoned_imgs, poisoned_labs = poison_data(imgs_cpu, labs_cpu, pdr=self.pdr, target_label=self.target_label)

        # 4. 重新封装
        # 注意：这里调用 TensorDataset 会再次使用被 utils.py 覆盖的那个类
        # 这是好事，保持了一致性
        batch_size = getattr(self.loader, 'batch_size', 32)
        num_workers = getattr(self.loader, 'num_workers', 0)
        ...
        # 替换 loader
        self.loader = DataLoader(poisoned_ds, batch_size=batch_size, shuffle=True, num_workers=num_workers)
        
        print(f"[Client_Backdoor {self.id}] Poisoned loader ready: PDR={self.pdr}, Scale={self.scale}, Size={len(poisoned_ds)}")
        return True

    def compute_weight_update(self, epochs=1, loader=None):
        """
        核心修正：使用 train_op 而不是 train_op_backdoor
        """
        # 如果外部没有传入 loader，就使用我们处理好的 self.loader (带毒)
        orig_loader = loader if loader is not None else self.loader

        # ⚠️ 【致命错误修正】 ⚠️
        # 之前调用的是 train_op_backdoor，它会再次修改数据且逻辑与我们不符。
        # 现在改用 train_op，它只会老老实实地在给定的 loader 上训练。
        # 因为 orig_loader 已经是带毒的了，所以这才是正确的做法。
        train_stats = train_op(self.model, orig_loader, self.optimizer, epochs)

        # --- Scaling 攻击逻辑 (放大恶意更新) ---
        user_grad = OrderedDict()
        if self.server_state is None:
            raise RuntimeError("server_state is None! Call synchronize_with_server first.")

        for name in self.W:
            # 1. 计算原始梯度： (Local_Weight - Global_Weight)
            raw_update = self.W[name].detach() - self.server_state[name]
            
            # 2. 恶意放大： Update * Scale
            scaled_update = raw_update * self.scale
            
            # 3. 写回模型： New_Weight = Global_Weight + Scaled_Update
            self.W[name].data = self.server_state[name] + scaled_update
