# Modified for FedTriad framework. 
# Based on FedREDefense: https://github.com/xyq7/FedREDefense
...
class Synthesizer:
    def __init__(self, network, args):
        ...
        hard_label = np.array(hard_label, dtype=int)  
    ...
    def synthesize_single(self, start_trajectories, end_trajectories, syn_data, syn_label, lr_syn, id, args, c_round):
        ...
        for it in range(iters):
            ...
            # Key fix: restore the original forward pipeline and remove mixed precision
            for step in range(self.syn_steps):
                ...
                # Remove autocast, use full precision
                ...
                # Memory optimization: clear cache every few steps
                if step % 5 == 0 and torch.cuda.is_available():
                    torch.cuda.empty_cache()

            # Compute loss (restore original calculation)
            ...
            # Remove or relax gradient clipping
            # torch.nn.utils.clip_grad_norm_([self.image_syn], max_norm=2.0)  # Optional: increase threshold

            self.optimizer_img.step()
            self.optimizer_lr.step()
            self.optimizer_label.step()
            
            # Memory optimization: clean up large objects promptly
            del student_net
